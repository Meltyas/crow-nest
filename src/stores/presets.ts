...
export async function togglePatrolEffectActive(effectId: string): Promise<void> {
  const current = get(presetsStore);
  const updated = {
    ...current,
...
export async function removeSituationalModifier(modifierId: string): Promise<void> {
  const current = get(presetsStore);
  const updated = {
    ...current,
    situationalModifiers: current.situationalModifiers.filter((m) => m.id !== modifierId),
  };
...
export async function deleteReputationPreset(reputationId: string): Promise<void> {
  await removeReputation(reputationId);
}

export async function deletePatrolEffectPreset(effectId: string): Promise<void> {
...
async function persistPresets(presets: PresetCollection): Promise<void> {
  try {
    ...
    if (!game || !game.settings) {
      console.warn("[Presets] Game settings not available yet, skipping persist");
      return;
    }
  ...
  try {
    ...
    if (saved) {
      ...
      const normalizedPresets: PresetCollection = {
        resources: presets.resources?.map(r => ({ ...r, active: r.active ?? false })) || [],
        reputations: presets.reputations?.map(r => ({ ...r, active: r.active ?? false })) || [],
        patrolEffects: presets.patrolEffects?.map(e => ({ ...e, active: e.active ?? false })) || [],
