<script lang="ts">
  ...
  function debugLogPatrolEffects() {
    console.log('=== DEBUG: Lista de Efectos de Patrulla ===');
    console.log('Active Patrol Effects (guard content):', activePatrolEffects);
    console.log('Available Presets (all presets):', availablePresets);
    console.log('Display Patrol Effects (current context):', displayPatrolEffects);
  ...
  function handleEditPatrolEffect(patrolEffect: any) {
    if (inPresetManager) {
      ...
      console.log('UnifiedPatrolEffects - Opening global dialog for patrol effect:', patrolEffect);
  ...
  function handleUsePatrolEffect(patrolEffect: any) {
    if (inPresetManager) {
      ...
      console.log('UnifiedPatrolEffects - Dispatching use patrol effect event:', patrolEffect);
  ...
  function handleEditItem(event: CustomEvent) {
    ...
    if (type === 'patrol-effect') {
      if (inPresetManager) {
        // Use global dialog when in preset manager
        console.log('UnifiedPatrolEffects - Opening global dialog for patrol effect:', item);
        const patrolEffect = {
  ...
  function togglePatrolEffectActiveState(data: { id: string, active: boolean }) {
    dispatch('activatePreset', data);
  }
